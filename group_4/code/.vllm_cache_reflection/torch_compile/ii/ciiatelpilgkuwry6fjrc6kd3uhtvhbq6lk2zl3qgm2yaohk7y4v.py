...
# kernel path: /data/jcy/zzs/zijinhua/.vllm_cache_reflection/torch_compile/wi/cwigbvdug3jkiuufwx65jfjeyvr2movgpkfrj6p6ixnuvjzupxks.py
...
#   %arg3_1 : Tensor "bf16[s27][1]cuda:0" = PlaceHolder[target=arg3_1]
...
#   %add_9 : Tensor "f32[s77, 1][1, 1]cuda:0"[num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%mean, 1e-06), kwargs = {})
#   %rsqrt : Tensor "f32[s77, 1][1, 1]cuda:0"[num_users=1] = call_function[target=torch.ops.aten.rsqrt.default](args = (%add_9,), kwargs = {})
...
#   %convert_element_type_1 : Tensor "f32[s27][1]cuda:0"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%arg3_1, torch.float32), kwargs = {})
#   %add_21 : Tensor "f32[s27][1]cuda:0"[num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%convert_element_type_1, 1.0), kwargs = {})
#   %mul_12 : Tensor "f32[s77, s27][s27, 1]cuda:0"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%mul_7, %add_21), kwargs = {})
#   %convert_element_type_2 : Tensor "bf16[s77, s27][s27, 1]cuda:0"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%mul_12, torch.bfloat16), kwargs = {})
...
class Runner:
    ...
    def call(self, args):
        ...
        assert_size_stride(arg3_1, (s27, ), (1, ))
        with torch.cuda._DeviceGuard(0):
            torch.cuda.set_device(0)
            buf1 = empty_strided_cuda((s77, s27), (s27, 1), torch.bfloat16)
            # Topologically Sorted Source Nodes: [x, pow_1, variance, add, rsqrt, x_1, float_2, add_1, x_2, x_3], Original ATen: [aten._to_copy, aten.pow, aten.mean, aten.add, aten.rsqrt, aten.mul]
            stream0 = get_raw_stream(0)
            triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_0.run(arg2_1, arg3_1, buf1, s27, s77, s27, stream=stream0)
            del arg2_1
            del arg3_1
        return (buf1, )
