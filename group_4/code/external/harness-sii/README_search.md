...
# 由 CPU 代你转传到 tmpfiles.org，最后再把得到的公网 URL 喂给 Lens
...
| `IMAGE_UPLOADER` | 可选 | 默认 `tmpfiles`（`0x0` 备选），仅 `/upload_image` 用 |
...
4. **图片上传**：本地图片经 CPU 端 `/upload_image` 转存到 tmpfiles.org（需代理），再喂给 Lens。GPU 侧无需直连外网图床。
